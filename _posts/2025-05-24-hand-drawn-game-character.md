...
![sketches]({{ '/assets/images/handdrawngame/sketches1L.jpg' | relative_url }})
...
![cutouts]({{ '/assets/images/handdrawngame/cutoutL.jpg' | relative_url }})
...
where the list can be loaded with sprites from the Unity UI, you simply drag and drop. Then, in order to cycle through the animations with a certain speed, I found it best to use the IEnumerator(), used for running coroutines. This function allows for breaks with a certain time extend, giving the choppy animation that I was looking for (Update() loop does not allow for breaks).
